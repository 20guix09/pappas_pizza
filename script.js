...
function init() {
  ...
  bindEvents();
  setupActiveCategoryEffects();
  handleSplashScreen();
}

function renderMenus() {
  function createCard(item, isFood = false, extraClass = "") {
    return `
      <div class="menu-card ${extraClass}">
        ...
        <div class="menu-card-footer">
          ...
          <button class="btn btn-primary" onclick="addDirectToCart('${item.id}', '${item.name.replace(/'/g, "\\'")}', ${item.price}, ${isFood})">
  ...
  const drinkGroups = [
    { title: "Refrigerantes 2L", items: drinksData.filter(d => d.name.includes("2L")) },
    { title: "Coca-Cola e opções menores", items: drinksData.filter(d => d.name.includes("350ml") || d.name.includes("600ml") || d.name.includes("1L")) },
    { title: "Sucos", items: drinksData.filter(d => d.name.toLowerCase().includes("suco")) },
    { title: "Cervejas e chopp", items: drinksData.filter(d => d.name.toLowerCase().includes("cerveja") || d.name.toLowerCase().includes("chopp")) }
  ];

  els.drinksGrid.innerHTML = drinkGroups
    .filter(group => group.items.length)
    .map(group => `
      <div class="drink-group">
        <h3 class="drink-group-title">${group.title}</h3>
        <div class="drink-grid">
          ${group.items.map(d => createCard(d, false, "drink-card")).join('')}
        </div>
      </div>
    `).join('');
...
function setupActiveCategoryEffects() {
  const trackedSections = document.querySelectorAll('#pizzas, #esfihas, #bebidas, #sobre');
  const navLinks = document.querySelectorAll('.main-nav a, .category-pill');
  ...
  navLinks.forEach(link => {
    link.addEventListener('click', () => {
      ...
      els.mainNav.classList.remove('active');
    });
  });
}
